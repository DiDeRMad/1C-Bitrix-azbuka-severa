	...
	if (is_array($YANDEX_EXPORT))
	{
		...
		foreach ($YANDEX_EXPORT as $ykey => $yvalue)
		{
			...
			while ($arAcc = $res->GetNext())
			{
				...
				if (intval($arAcc["PREVIEW_PICTURE"]) > 0 || intval($arAcc["DETAIL_PICTURE"]) > 0)
				{
					// Приоритетное использование PREVIEW_PICTURE
					$pictNo = intval($arAcc["PREVIEW_PICTURE"]);
					if ($pictNo <= 0) $pictNo = intval($arAcc["DETAIL_PICTURE"]);
					...
					if (is_array($arPictInfo))
					{
						...
						$strTmpOff .= "<picture>" . htmlspecialchars($strFile) . "</picture><pp></pp>\n\n\n\n";
					}
				}
