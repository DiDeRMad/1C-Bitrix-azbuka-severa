...
if (!function_exists("get_measure_info"))
{
    function get_measure_info($measureId)
    {
        static $mList = false;
        ...
        return isset($mList[ $measureId ]) ? $mList[ $measureId ] : "";
    }
}

if (!function_exists("get_measure_text"))
{
    function get_measure_text($productId)
    {

        $productInfo = CCatalogProduct::GetByID($productId);

        $measureInfo = get_measure_info( $productInfo["MEASURE"] );

        $weight = intval($productInfo["WEIGHT"]);

        if ( $measureInfo["SYMBOL_INTL"] == "kg" )
        {
            $weight = round( ($weight/1000), 1 );
        }

        return '<param name="Вес2" unit="'. $measureInfo["SYMBOL_RUS"] .'">'. $weight .'</param>' ."\n".
            '<param name="Единица измерения">'. $measureInfo["SYMBOL_RUS"] .'</param>';
    }
}


if (!function_exists("get_measure_name"))
{
	function get_measure_name($measureId)
	{
		static $mList = false;
		...
		return isset($mList[ $measureId ]) ? $mList[ $measureId ]["SYMBOL_RUS"] : "";
	}
}
...
if ($bParam)
{
	if (!empty($arProperty['DESCRIPTION']) && is_array($arProperty['DESCRIPTION'])) {
		foreach ($arProperty['DESCRIPTION'] as $key => $description) {
			// Получаем значение из description (например, "56,6 гр.")
			$value = isset($arProperty['~VALUE'][$key]) ? trim($arProperty['~VALUE'][$key]) : '';

			// Очистка name и value
			$value = htmlspecialchars(trim($value), ENT_XML1 | ENT_QUOTES, 'UTF-8');
			$description = htmlspecialchars(trim($description), ENT_XML1 | ENT_QUOTES, 'UTF-8');

			// Если описание пустое, пропускаем
			if ($description === '') continue;

			// Извлекаем единицу измерения из description
			$unit = '';
			if (preg_match('/(гр\.|ккал)$/i', $description, $matches)) {
				$unit = $matches[1]; // Единица измерения
				$description = preg_replace('/\s?(гр\.|ккал)$/i', '', $description); // Убираем единицу из description
			}

			// Очистка unit
			$unit = htmlspecialchars(trim($unit), ENT_XML1 | ENT_QUOTES, 'UTF-8');

			// Формируем параметр
			$strProperty .= '<param name="'.yandex_text2xml($value, $options).'" unit="'.yandex_text2xml($unit, $options).'">'.yandex_text2xml($description, $options).'</param>';
		}
	}
	else {
		if (is_array($description))
		{
			if (!empty($value))
			{
				foreach ($value as $key => $val)
				{
					if ($val != '')
					{
						// Формируем строку с параметром
						$strProperty .= $strProperty ? "\n" : "";

						// Получаем название параметра из массива $description
						$param_name = yandex_text2xml($description[$key], $options);
						$param_value = yandex_text2xml($val, $options);

						// Используем $key как единицу измерения, если она присутствует
						$unit = '';
						if (isset($description[$key . '_unit'])) {
							$unit = yandex_text2xml($description[$key . '_unit'], $options); // например, для "гр."
						}

						// Если unit не задан, указываем по умолчанию, например "гр."
						if (!$unit) {
							$unit = "гр."; // или любая другая единица
						}

						// Формируем параметр с атрибутом unit
						$strProperty .= '<param name="'.$param_name.'" unita="'.$unit.'">'.$param_value.'</param>';
		...
		if ($value != '')
			{
				// Используем название свойства для параметра
				$param_name = yandex_text2xml($iblockProperty['NAME'], $options);
				$param_value = yandex_text2xml($value, $options);

				// Указываем единицу измерения, если она задана
				$unit = "гр."; // Или аналогичная логика для поиска единицы измерения

				// Формируем параметр с атрибутом unit
				if (strpos($param_name, 'Вес для торг. предл') !== false) {
					// Если $param_name содержит "Вес для торг. предл", заменяем на "counter" и убираем unit
					$strProperty .= '<param name="counter">' . $param_value . '</param>';
				} else {
					// Иначе формируем стандартный <param>
					if (strpos($param_name, 'Хит продаж') !== false) {
						// Если параметр "Хит продаж", меняем на "Hit" и убираем unit
						$strProperty .= '<param name="Hit">' . $param_value . '</param>';
					} else {
						// Стандартное добавление параметра
						// weight надо пропустить
						$strProperty .= '<param name="' . $param_name . '" unit="' . $unit . '">' . $param_value . '</param>';
					}
				}
			}
		}
...
{
    // Если нет параметра, генерируем стандартный блок
			...
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					foreach ($row['OFFERS'] as $offer)
					{
                        ...
                        // Добавляем проверку на наличие PREVIEW_PICTURE
                        if (empty($picture) && !empty($row['PREVIEW_PICTURE'])) {
                            $picture = CFile::GetPath($row['PREVIEW_PICTURE']);
                        }
                        if (!empty($picture)) {
							$itemsContent .= "<picture>" . htmlspecialchars($picture) . "</picture><pt></pt>\n\n\n\n";
                        }
						unset($picture);
						
						if (!empty($row['PREVIEW_PICTURE'])) {
                            $picture2 = CFile::GetPath($row['PREVIEW_PICTURE']);
                        }
                        if (!empty($picture2)) {
                            $itemsContent .= "<pict2>" . htmlspecialchars($picture2) . "</pict2>\n\n\n\n";
                        }
						unset($picture2);
						...
						foreach ($arYandexFields as $key)
						{
							switch ($key)
							{
								case 'name':
									...
									$nameH1 = CIBlockElement::GetProperty($offer['IBLOCK_ID'], $offer['ID'], [], ['CODE' => 'NAME_H1'])->Fetch();
									$itemsContent .= "<name>".yandex_text2xml($nameH1['VALUE'] ?? $offer['NAME'], $itemOptions)."</name>\\n";
									break;
								case 'description':
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
					...
					/*if (empty($row['PICTURE']) && !empty($row['PREVIEW_PICTURE'])) {
						$row['PICTURE'] = CFile::GetPath($row['PREVIEW_PICTURE']);
					}
					if (!empty($row['PICTURE'])) {
						$itemsContent .= "<picture>" . htmlspecialchars($row['PICTURE']) . "</picture>\n\n\n\n";
					}*/


					if (!empty($row['PREVIEW_PICTURE'])) {
						$itemsContent .= "<picture>" . ($itemUrlConfig['USE_DOMAIN'] ? $usedProtocol.$site['SERVER_NAME'] : ''). htmlspecialchars(CFile::GetPath($row['PREVIEW_PICTURE'])) . "</picture>\n\n\n\n";
					}

					$productInfo = CCatalogProduct::GetByID($row['ID']);

					$itemsContent .= get_measure_text($row['ID']) ."\n";
					//$itemsContent .= "<CAT_MEASURE>". serialize($productInfo) ."</CAT_MEASURE>";
					...
					foreach ($arYandexFields as $key)
					{
						switch ($key)
						{
							case 'name':
								...
								$nameH1 = CIBlockElement::GetProperty($row['IBLOCK_ID'], $row['ID'], [], ['CODE' => 'NAME_H1'])->Fetch();
								$itemsContent .= "<name>".yandex_text2xml($nameH1['VALUE'] ?? $row['NAME'], $itemOptions)."</name>\\n";
								break;
							case 'description':
