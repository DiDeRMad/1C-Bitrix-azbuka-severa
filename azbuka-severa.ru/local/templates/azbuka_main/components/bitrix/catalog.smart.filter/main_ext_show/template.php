                                    ...
                                    <div class="form-group" style="width: calc(50% - 10px);">
                                        <div class="smart-filter-input-container">
                                            <input
                                                    ...
                                                    data-min="<?= $arItem["VALUES"]["MAX"]["HTML_VALUE"]?>"
        ...
        foreach ($arResult["ITEMS"] as $key => $arItem) {
            ...
            case "A":
                ...
                <div class="filter__item filter__item--price">
                    <div class="filter__name js-filter-toggle">
                        <div class="filter__label">Цена</div>
                        <div class="filter__btn">
                            <svg class="icon--24">
                                <use xlink:href="<?= SITE_TEMPLATE_PATH ?>/assets/img/sprite.svg#caret-up-icon"></use>
                            </svg>
                        </div>
                    </div>
                    <div class="filter__content" style="">
                        <div class="filter__item--range">
                            <div class="price-range-slider"></div>
                        </div>
                        <div class="filter__item--input">
                            <label for="price-start" class="filter__item--label">
                                <span>от</span>
                                <input class="range-input range-input__start"
                                       type="number"
                                       name="<? echo $arItem["VALUES"]["MIN"]["CONTROL_NAME"] ?>"
                                       id="<? echo 'price-start' ?>"
                                ...
                                />                            </label>
                            <label for="price-end" class="filter__item--label">
                                <span>до</span>
                                <input
                                        class="range-input range-input__end"
                                        type="number"
                                        name="<? echo $arItem["VALUES"]["MAX"]["CONTROL_NAME"] ?>"
                                        id="<? echo 'price-end' ?>"
            ...
            ?>
                <?
                $bParams = ( count($arItem["VALUES"]) > 7? true : false );//количество чекбокосов
                $cl = ($bParams ?" ext-scroll":"");
                ?>
                <div class="filter__item">
                    <div class="filter__refresh">Сбросить фильтр</div>
                    <div class="filter__name js-filter-toggle">
                        <div class="filter__label"><?= $arItem['NAME'] ?></div>
                        <div class="filter__btn">
                            <svg class="icon--24">
                                <use xlink:href="/local/templates/azbuka_main/assets/img/sprite.svg#caret-up-icon"></use>
                            </svg>
                        </div>
                    </div>
                    <div class="filter__content<?=$cl?>">
                        <? foreach ($arItem["VALUES"] as $val => $ar): ?>
                            <label data-role="label_<?= $ar["CONTROL_ID"] ?>" class="filter__content-checkbox"
                                   for="<? echo $ar["CONTROL_ID"] ?>">
                                <input
                                        type="checkbox"
            ...
            <div class="col smart-filter-button-box">
                <div class="smart-filter-block">
                    <div class="smart-filter-parameters-box-container">
                        ...
                        <button id="prepare_filter" type="button" class="btn btn--blue"  value="<?= GetMessage("CT_BCSF_SET_FILTER") ?>"><?= GetMessage("CT_BCSF_SET_FILTER") ?></button>
                                <input
                                    style="display: none;"
                                    class="btn btn-link"
                                    type="submit"
                        ...
                        <div class="smart-filter-popup-result <? if ($arParams["FILTER_VIEW_MODE"] == "VERTICAL") echo $arParams["POPUP_POSITION"] ?>"
                             id="modef" <? if (!isset($arResult["ELEMENT_COUNT"])) echo 'style=""'; ?>
...
<script type="text/javascript">
    ...
    /* инициализация скролла для чекбоксов*/
    initScrollBar($(".ext-scroll"));
</script>
<?php
    $this->SetViewTarget('catalog_filtered-relink');
    ...
    foreach ($arResult['ITEMS'] as $key => $item) {
        if (strtolower($item['CODE']) == 'sort_price') continue;

        if (strpos($dir, '/filter/' . strtolower($item['CODE'])) !== false) {
            foreach ($item['VALUES'] as $valueKey => $value) {
                if ($value['CHECKED'] == '1') continue;

                foreach ($arDir as &$dirElem) {
                    if (preg_match('/' . strtolower($item['CODE']) . '/', $dirElem)) {
                        $dirElem = strtolower($item['CODE']) . '-is-' . strtolower($value['URL_ID']);
                    }
                }
                $link = implode('/', $arDir);
                $arSeeMore[] = [
    ...
    if (!empty($arSeeMore)) {
        echo '<div class="container"><span>Смотрите также:';
        foreach ($arSeeMore as $linkMore) {
            echo '<a href="' . $linkMore['LINK'] . '" style="margin-left:10px;  color: #1b389e; font-weight: 600;">' . $linkMore['NAME'] . '</a> ';
        }
        echo '</span><br /><br />';
    }
