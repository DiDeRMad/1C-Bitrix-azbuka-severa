...
?>
	...
	<form id="bx_subscribe_subform_<?=$buttonId?>" role="form" class="123" method="post" action="<?=$arResult["FORM_ACTION"]?>">
		<?=bitrix_sessid_post()?>

        <div class="subscribe__form">
            <input id="subs_email" type="email" name="SENDER_SUBSCRIBE_EMAIL" value="<?=$arResult["EMAIL"]?>" class="input" placeholder="E-mail">
            <button style="display: none;" id="bx_subscribe_btn_<?=$buttonId?>" type="submit" class="btn btn--blue send-subscribe">Подписаться</button>
...
<script>
    $('.captcha-check').on('click', function (e) {
        ...
        grecaptcha.ready(function () {
            grecaptcha.execute('<?=RECAPTHA_SITEKEY?>', {action: 'subscribe'}).then(function(token) {
