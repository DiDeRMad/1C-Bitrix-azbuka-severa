...
<div class="row bx-<?=$arParams['TEMPLATE_THEME']?>">
	<div class='<?=($isSidebar ? 'col-md-9 col-sm-8' : 'col')?>'>
        ...
        //подменям в заголовках старницы килограммы на граммы и приводим к нормальному виду
        //$arUnit берется из result_modifier.php bitrix:catalog.element
        global $arUnit;

        $strCurrentTitle = $APPLICATION->GetPageProperty("title");
        //что
        $arWhat = ["#WEIGHT#", '#WEIGHT_TYPE#'];
        //на что
        $arONWhat = [$arUnit['weight'], $arUnit['type']];
        //меняем
        $APPLICATION->SetPageProperty("title", str_replace($arWhat, $arONWhat, $strCurrentTitle));
